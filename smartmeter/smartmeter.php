...
$tcpsocket = stream_socket_server("tcp://0.0.0.0:58881", $errno, $errstr);
if (!$tcpsocket) {
...
function writeEnergyDatabase($gas_used, $kwh_used1, $kwh_used2, $kwh_provided1, $kwh_provided2, $kw_using, $kw_providing)
{
  ...
  if ($Mysql_con)
  {        
      if (mysql_select_db("electricitymeter", $Mysql_con))
      {
        if (mysql_query("INSERT INTO `electricitymeter` (kw_using, kw_providing, kwh_used1, kwh_used2, kwh_provided1, kwh_provided2)
      ...
      if (mysql_select_db("gasmeter", $Mysql_con))
      {
        if (mysql_query("INSERT INTO `gasmeter` (m3, m3h) VALUES ($gas_used, 0)"))
        {
          return 1;
        }
      }
